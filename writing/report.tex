...
\begin{document}
    ...
    The heating step occurs very consistently across different realizations. The entanglement of states with different
    sizes $N$ grows with the same rate, before individual system sizes converge to their maximum entanglement. The maximum
    entanglement we reach during the heating step is proortional to $N$:

    \begin{equation}
        \text{max}\braket{S_{N_A}} \propto N.
    \end{equation}

    This is in line with the known sacling $S\propto \log_2d$ \cite{Page:1993:AverageEntropy} of the entropy of a random
    quantum state, where $d$ is the dimension of the local Hilbert space (recall that here, $d=2^{N_1}$). The rate of entanglement
    ...
    The entanglement cooling step does not depend on the value of $\beta$; for $\beta\in [1,10]$, cooling occurs at the
    ...
    We move on now to the central result of this work, which concerns irreversibility: The entanglement cooling step
    does not always succeed. If the creation of entanglement were reversible, cooling would always be able to lower
    the entanglement back (or close to) zero; this is not the case, however. Specifically, entanglement cooling fails,
    whenever the gate set that is used is universal. Entanglement cooling occurs at the same rate regardless of system
    size $N$ and whether it succeeds or fails, however disentangling begins later during the cooling process for larger
    systems. See also Figure~\ref{results:fig:therm_spped_iSWAP}.
